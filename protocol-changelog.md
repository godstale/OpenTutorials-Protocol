...
- **3D 모델 뷰어/플레이어 임베드 `vivo:3d` 신설 (제4.3.7절)**:
  - 강좌 카드 본문에 GLB 3D 모델을 삽입하여 회전/줌/팬(OrbitControls), 조명/배경 제어, 자동 회전 지원.
...
- **코드 IDE/런타임 실습 임베드 `vivo:runtime` 신설 (제4.3.8절)**:
  ...
  - **뷰 모드(`view: "split" | "preview" | "editor"`) 및 카드 레이어 확장 옵션 지원**:
    - `extend`: card가 표시되는 레이어 전체를 사용하고 다른 마크다운 본문은 가려짐 (토글 가능).
    - `extend_only`: card 레이어 전체를 사용하도록 고정 (축소 불가).
    - `extend_visual`: **`vivo:runtime` 전용**. 에디터/툴바 없이 실행 결과(Visual/Web iframe 등)만 카드 레이어 전체에 고정 (3D/인트로 씬 연출 지원).
- **전체 임베드 컴포넌트(`vivo:video`, `vivo:motion`, `vivo:lottie`, `vivo:3d`, `vivo:runtime`)에 `extend` 및 `extend_only` 옵션 지원**:
  - 모니터/윈도우 전체를 덮는 대신 card가 표시되는 레이어 전체를 임베드가 채우도록 하는 레이아웃 확장 지원.
- **제6조 검증 규칙에 `vivo:3d` 및 `vivo:runtime` 검증 항목 추가 (5번 항목)**:
  ...
  - `vivo:runtime`: 구분선(`^---(?:[ \t]+(\S.*?))?[ \t]*$`) 파싱, 헤더 JSON 검증, `lang` × `mode` 허용 조합표 검증, `view` allowlist 검증, `extend`/`extend_only`/`extend_visual` boolean 검증 (`extend_visual`은 `vivo:runtime` 외 타 임베드에서 사용 시 거부), 실행 명령 금지 키 차단, 파일명 형식(`^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$`) 및 경로 탈출 방지, 네이티브 모드 자동 실행 금지, Python WASM 패키지 허용 목록 검증.
